...
你好，我是小 G。前几天分享 [Claude Code 使用技巧](/ai-coding/practices/claudecode-tips.html) 时，我简单介绍了 `CLAUDE.md`。有 G 友在评论区问，这个文件能不能单独写一篇。
...
我的 [一文搞懂 Harness Engineering](/ai/agent/harness-engineering.html) 还介绍过一个例子：OpenAI 的 `AGENTS.md` 大约只有 100 行，主要用于指向 docs/ 目录下更具体的设计文档、架构图、执行计划和质量评级。Agent 先读取入口文件，处理到相关任务时再加载详细资料，避免一开始就把所有内容放进上下文。
...
![Spec Coding 规范驱动编程流水线](https://oss.javaguide.cn/github/offerkit/ai/coding/spec-coding-pipeline-flow.png)

上图中的 `requirements.md` 是该工作流在 `Specify` 阶段生成的需求文件；其他团队也可能把同类任务规格集中写在 `SPEC.md`，两者不是通用的固定别名。

我在[Spec Coding 规范驱动编程实战：从 Vibe Coding 到 AI 代码规范](/ai-coding/practices/spec-coding.html)这篇文章中有详细介绍。
...
![上下文为什么会失效](https://oss.javaguide.cn/github/offerkit/ai/context-engineering/why-does-the-following-content-fail.png)
...
![andrej-karpathy-claudemd](https://oss.javaguide.cn/github/offerkit/ai/coding/claudecode/andrej-karpathy-claudemd.png)
...
我的 [interview-guide](/zhuanlan/interview-guide.html) 使用的是项目级 `CLAUDE.md`。根目录文件保留技术栈、常用命令、分层边界、异常处理、事务规则和禁止清单，详细规范再交给 `.claude/rules/`。
...
![Claude Code PreToolUse Hook](https://oss.javaguide.cn/github/offerkit/ai/coding/claude-code-runs-rm-rf-tmp-build-what-happens.svg)
...
![CLAUDE.md 组织方式演进](https://oss.javaguide.cn/github/offerkit/ai/coding/claudecode/claude-md-best-practices-scaling-evolution.png)
...
![CLAUDE.md 维护决策流程](https://oss.javaguide.cn/github/offerkit/ai/coding/claudecode/claude-md-best-practices-maintenance-flow.png)
