...
![ Vibe Coding 技巧分享读者评论](https://oss.javaguide.cn/github/offerkit/ai/coding/claudecode/vibe-coding-practices-comments.png)
...
![Claude Code Git Worktree](https://oss.javaguide.cn/github/offerkit/ai/coding/claude-code-git-worktree.png)
...
![Spec Coding 四步流水线](https://oss.javaguide.cn/github/offerkit/ai/coding/spec-coding-pipeline-flow.png)

小任务写清楚目标、约束和验收就够了；中等任务再补接口格式、错误码、表结构；大一点的需求，再拆成 `requirements.md`、`design.md`、`tasks.md`。没必要一上来就把流程拉满，不然你会先被文档劝退。

关于 Spec Coding 的详细介绍，可以参考：[Spec Coding 规范驱动编程实战：从 Vibe Coding 到 AI 代码规范](/ai-coding/practices/spec-coding.html)。
...
![多智能股票分析项目中的 CLAUDE.md 和 AGENTS.md](https://oss.javaguide.cn/github/offerkit/ai/coding/claude-agents-md.png)
...
![Agent 执行链路](https://oss.javaguide.cn/github/offerkit/ai/skills/skills-agent-execution-link.png)
...
![SKILL.md 正文最好控制在 500 行以内](https://oss.javaguide.cn/github/offerkit/ai/skills/keep-skill-md-content-under-500-lines-for-best-performance.png)
...
![DeepSeek V4 Benchmark 数据](https://oss.javaguide.cn/github/offerkit/ai/coding/deepseek-v4/v4-benchmark.png)
...
**第二，长任务要及时压缩。** Claude Code 可以用 `/compact` 压缩上下文，用 `/clear` 清空上下文（详细用法参考 [Claude Code 核心命令详解](/ai-coding/practices/claudecode-commands.html)）；Codex 或其他 Agent 也有类似的摘要、压缩、重开机制。压缩是为了保留重点（如：架构决策、已改文件、未解决问题、失败命令和下一步任务），丢掉重复对话和已经消化过的工具输出。
...
![多 Agent 并行会话](https://oss.javaguide.cn/github/offerkit/ai/coding/claudecode/multi-agent-parallel-sessions.png)
