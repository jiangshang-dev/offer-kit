...
![oh-my-pi Hashline 官方说明截图](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-hashline-doc.png)
...
我顺手拿 OfferKit 的一个 issue 试了下。它先读 issue，再去仓库里找对应 Markdown，接着顺着图片链接继续读。

![oh-my-pi 读取 GitHub issue 并追踪仓库文件](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-github-issue-read.png)
...
![oh-my-pi 在模型面板里把同一个模型设置为不同角色](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-model-role-actions.png)
...
![oh-my-pi 第一次启动选择模型 provider](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-setup-provider-login-kimi.png)

![Kimi Code 会员权益页面](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-kimi-code-home.png)

然后切到 Web search，选择 `web_search` 工具优先使用哪个搜索后端。当前项目已扩展到约 25 个后端，静态列出名称很快会过期；选 `Auto` 时会从已经配置好的后端中选择，手动模式以当前 Setup 页面为准。

![oh-my-pi 第一次启动选择 Web search provider](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-setup-web-search.png)

这一步不用纠结太久。先把一个主模型和一个搜索 provider 跑通，比一上来把所有账号都接进去更稳。

配置完回到主界面后，我这里模型已经直接选好了，左侧显示的是 `DeepSeek V4 Flash`。

![oh-my-pi 启动后自动选中默认模型](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-welcome-default-model.png)
...
![oh-my-pi 解释模型为什么会自动配置](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-model-auto-config-reason.png)

后面想换模型也不用重启，直接用 `/model`。它只会展示已经有可用凭据的模型，上面还能按 provider 切 tab。我这里能看到 DeepSeek、Z.AI、Ollama、LM Studio、llama.cpp 这些入口。

![oh-my-pi 使用 model 命令切换模型](https://oss.javaguide.cn/github/offerkit/ai/coding/oh-my-pi/oh-my-pi-model-switch.png)
