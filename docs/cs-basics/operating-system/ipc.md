...
![进程地址空间隔离导致进程间通信需要借助内核提供的 IPC 机制](https://oss.javaguide.cn/github/offerkit/java/new-features/ipc-why-ipc.png)

## IPC 到底在解决什么？

![IPC 需要同时解决数据传递、同步控制、命名寻址和权限检查等问题](https://oss.javaguide.cn/github/offerkit/java/new-features/what-problem-does-ipc-solve.png)
...
![管道通过内核缓冲区在父进程和子进程之间传递单向字节流](https://oss.javaguide.cn/github/offerkit/java/new-features/ipc-pipe-flow.png)
...
![共享内存让多个进程映射同一块物理内存，但仍需要信号量、futex 等同步机制配合](https://oss.javaguide.cn/github/offerkit/java/new-features/ipc-shared-memory.png)
...
![Android Binder 通过 AIDL、Parcel、Binder 驱动和 Service Manager 将 IPC 封装成系统服务调用](https://oss.javaguide.cn/github/offerkit/java/new-features/android-binder-turning-ipc-into-system-service-calls.png)
...
![常见 IPC 方式在数据形态、消息边界、大数据传输和跨机器能力上的横向对比](https://oss.javaguide.cn/github/offerkit/java/new-features/ipc-ipc-comparison.png)
...
![根据数据量、消息边界、跨机器和进程亲缘关系选择合适的 IPC 方式](https://oss.javaguide.cn/github/offerkit/java/new-features/ipc-ipc-selection.png)
