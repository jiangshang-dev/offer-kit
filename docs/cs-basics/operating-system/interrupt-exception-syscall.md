...
![中断、异常与系统调用关系图](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/ecf-kernel-entry-map.webp)
...
![用户态内核态切换与上下文切换对比图](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/kernel-mode-vs-context-switch.webp)

## `read()` 的系统调用路径

以 Linux x86-64 上的 `read(fd, buf, count)` 为例，业务代码一般调用的是 glibc 包装函数，不会自己写汇编。

![read 系统调用流程图](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/read-syscall-path.webp)
...
![Page Fault 处理分支图](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/page-fault-branching.webp)
