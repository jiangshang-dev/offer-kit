...
![死锁场景示意图：线程 A 持有 resource1 并等待 resource2，线程 B 持有 resource2 并等待 resource1，等待链形成闭环](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/dead-lock-deadlock-scenario.png)
...
![死锁四个必要条件示意图：互斥、请求与保持、非抢占、循环等待同时成立才会形成死锁](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/dead-lock-four-conditions.png)
...
![资源分配图与等待图示意图：资源分配图包含线程和资源节点，等待图只保留线程之间的等待关系](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/dead-lock-resource-allocation-graph.png)
...
![死锁处理策略图：预防、避免、检测、恢复四类方法的作用位置和工程常见程度](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/dead-lock-strategies.png)
...
![jconsole](https://oss.javaguide.cn/github/offerkit/java/concurrent/jdk-home-bin-jconsole.png)

连接目标 Java 进程后，进入“线程”页面，点击“检测死锁”。

![jconsole 检测死锁](https://oss.javaguide.cn/github/offerkit/java/concurrent/jconsole-check-deadlock.png)

如果目标进程里存在 Java 线程死锁，JConsole 会把相关线程单独列出来。

![jconsole 检测到死锁](https://oss.javaguide.cn/github/offerkit/java/concurrent/jconsole-check-deadlock-done.png)
...
![死锁、饥饿和活锁对比图：死锁表现为等待环，饥饿表现为长期拿不到资源，活锁表现为持续动作但没有进展](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/dead-lock-deadlock-vs-starvation-livelock.png)
...
- [OfferKit：操作系统常见面试题总结（上）](https://github.com/jiangshang-dev/offerkit)
