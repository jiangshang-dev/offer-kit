...
![临界区保护访问协议示意图：多个线程通过统一加锁入口访问共享状态，绕开锁或更换锁对象都会破坏互斥关系](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/os-lock-critical-section.png)
...
![mutex 和 spinlock 等待方式对比：mutex 在可阻塞路径中睡眠等待，spinlock 在不能睡眠的短路径中短暂忙等](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/os-lock-mutex-spinlock.png)
...
![semaphore 管理有界缓冲区资源数量示意图：empty_slots 记录空位数量，filled_slots 记录可消费数量，buffer_mutex 保护缓冲区结构](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/os-lock-semaphore-buffer.png)
...
![condition variable 等待条件成立流程图：线程在 while 中检查共享状态，条件不满足时释放 mutex 并睡眠，被 signal 唤醒后重新检查条件](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/os-lock-condition-variable.png)
...
![futex 用户态快路径与内核慢路径示意图：无竞争时通过用户态原子操作拿锁，竞争失败后进入 FUTEX_WAIT，释放时通过 FUTEX_WAKE 唤醒等待线程](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/os-lock-futex.png)
...
![用户态锁和内核锁的上下文差异示意图：用户态主要关注线程协作，内核态还要判断能否睡眠、能否抢占、是否处于中断路径以及是否跨 CPU 共享](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/os-lock-kernel-context.png)
