...
![网络读取中的两个阶段：先等待网卡数据进入内核缓冲区，再通过 copy_to_user 拷贝到用户缓冲区](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/io-multiplexing-io-two-phases.png)
...
![五种 I/O 模型对比：阻塞 I/O、非阻塞 I/O、I/O 多路复用、信号驱动 I/O 和异步 I/O](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/io-multiplexing-five-io-models.png)
...
![epoll 内部架构：epoll_ctl 维护 interest list，fd 就绪后通过回调进入 ready list，epoll_wait 返回就绪事件](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/io-multiplexing-epoll-architecture.png)
...
![水平触发和边缘触发对比：LT 在数据未读完时会持续通知，ET 只在状态变化时通知一次](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/io-multiplexing-lt-vs-et.png)
...
![select、poll 和 epoll 对比：数据结构、fd 限制、每次等待传参、查找就绪 fd 的开销和触发模式](https://oss.javaguide.cn/github/offerkit/cs-basics/operating-system/io-multiplexing-select-poll-epoll.png)
...
![Selector 选择器工作示意图](https://oss.javaguide.cn/github/offerkit/java/nio/selector-channel-selectionkey.png)
