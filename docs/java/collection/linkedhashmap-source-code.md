...
![LinkedHashMap 逻辑结构](https://oss.javaguide.cn/github/offerkit/java/collection/linkhashmap-structure-overview.png)
...
![](https://oss.javaguide.cn/github/offerkit/java/collection/lru-cache.png)
...
> **🐛 修正（参见：[issue#2147](https://github.com/jiangshang-dev/offerkit/issues/2147)）**：
>
> 链表上的节点个数达到树化的阈值是 8 而非 7。因为源码的判断是从链表初始元素开始遍历，下标是从 0 开始的，所以判断条件设置为 8-1=7，其实是迭代到尾部元素时再判断整个链表长度大于等于 8 才进行树化操作。
>
> ![](https://oss.javaguide.cn/github/offerkit/java/jvm/LinkedHashMap-putval-TREEIFY.png)
...
![LinkedHashMap 和 HashMap 之间的关系](https://oss.javaguide.cn/github/offerkit/java/collection/map-hashmap-linkedhashmap.png)
...
![LinkedHashMap 移动元素 13 到链表尾部](https://oss.javaguide.cn/github/offerkit/java/collection/linkedhashmap-get.png)
...
![LinkedHashMap 删除元素 13](https://oss.javaguide.cn/github/offerkit/java/collection/linkedhashmap-remove.png)
...
![LinkedHashMap 中插入新元素 19](https://oss.javaguide.cn/github/offerkit/java/collection/linkedhashmap-after-insert-1.png)

移除的步骤很简单，查看链表首节点是否存在，若存在则断开首节点和后继节点的关系，并让首节点指针指向下一节点，所以 head 指针指向了 12，节点 10 成为没有任何引用指向的空对象，等待 GC。

![LinkedHashMap 中插入新元素 19](https://oss.javaguide.cn/github/offerkit/java/collection/linkedhashmap-after-insert-2.png)
