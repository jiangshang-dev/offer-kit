...
> 本文来自公号程序猿阿星投稿，OfferKit 对其做了补充完善。

## 前言

MySQL 日志 主要包括错误日志、查询日志、慢查询日志、事务日志、二进制日志几大类。其中，比较重要的还要属二进制日志 binlog（归档日志）和事务日志 redo log（重做日志）和 undo log（回滚日志）。

![](https://oss.javaguide.cn/github/offerkit/01.png)
...
![](https://oss.javaguide.cn/github/offerkit/02.png)
...
![](https://oss.javaguide.cn/github/offerkit/03.png)
...
![](https://oss.javaguide.cn/github/offerkit/04.png)
...
![](https://oss.javaguide.cn/github/offerkit/05.png)
...
![](https://oss.javaguide.cn/github/offerkit/06.png)

为`0`时，如果 MySQL 挂了或宕机可能会有`1`秒数据的丢失。

#### innodb_flush_log_at_trx_commit=1

![](https://oss.javaguide.cn/github/offerkit/07.png)
...
![](https://oss.javaguide.cn/github/offerkit/09.png)
...
![](https://oss.javaguide.cn/github/offerkit/10.png)
...
![](https://oss.javaguide.cn/github/offerkit/11.png)

如果 `write pos` 追上 `checkpoint` ，表示**日志文件组**满了，这时候不能再写入新的 redo log 记录，MySQL 得停下来，清空一些记录，把 `checkpoint` 推进一下。

![](https://oss.javaguide.cn/github/offerkit/12.png)
...
![](https://oss.javaguide.cn/github/offerkit/01-20220305234724956.png)
...
![](https://oss.javaguide.cn/github/offerkit/02-20220305234738688.png)

同步数据时，会执行记录的`SQL`语句，但是有个问题，`update_time=now()`这里会获取当前系统时间，直接执行会导致与原库的数据不一致。

为了解决这种问题，我们需要指定为`row`，记录的内容不再是简单的`SQL`语句了，还包含操作的具体数据，记录内容如下。

![](https://oss.javaguide.cn/github/offerkit/03-20220305234742460.png)
...
![](https://oss.javaguide.cn/github/offerkit/04-20220305234747840.png)
...
![](https://oss.javaguide.cn/github/offerkit/05-20220305234754405.png)
...
![](https://oss.javaguide.cn/github/offerkit/06-20220305234801592.png)
...
![](https://oss.javaguide.cn/github/offerkit/01-20220305234816065.png)
...
![](https://oss.javaguide.cn/github/offerkit/02-20220305234828662.png)

由于 binlog 没写完就异常，这时候 binlog 里面没有对应的修改记录。因此，之后用 binlog 日志恢复数据时，就会少这一次更新，恢复出来的这一行`c`值是`0`，而原库因为 redo log 日志恢复，这一行`c`值是`1`，最终数据不一致。

![](https://oss.javaguide.cn/github/offerkit/03-20220305235104445.png)

为了解决两份日志之间的逻辑一致问题，InnoDB 存储引擎使用**两阶段提交**方案。

原理很简单，将 redo log 的写入拆成了两个步骤`prepare`和`commit`，这就是**两阶段提交**。

![](https://oss.javaguide.cn/github/offerkit/04-20220305234956774.png)

使用**两阶段提交**后，写入 binlog 时发生异常也不会有影响，因为 MySQL 根据 redo log 日志恢复数据时，发现 redo log 还处于`prepare`阶段，并且没有对应 binlog 日志，就会回滚该事务。

![](https://oss.javaguide.cn/github/offerkit/05-20220305234937243.png)

再看一个场景，redo log 设置`commit`阶段发生异常，那会不会回滚事务呢？

![](https://oss.javaguide.cn/github/offerkit/06-20220305234907651.png)
