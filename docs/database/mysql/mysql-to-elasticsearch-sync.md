...
![MySQL 到 ES 全量与增量衔接](https://oss.javaguide.cn/github/offerkit/database/es/mysql-es-full-incremental-sync.webp)
...
![应用层同步双写](https://oss.javaguide.cn/github/offerkit/database/es/es-mq-synchronization-synchronous-double-write.png)
...
![定时同步](https://oss.javaguide.cn/github/offerkit/database/es/es-mq-synchronization-scheduled-task.png)
...
![Canal 工作原理](https://oss.javaguide.cn/github/offerkit/open-source-project/canal-overview.png)

Canal Server 的核心职责是增量日志订阅和解析。下游可以使用 Canal Client 直接消费，也可以让 Canal 把消息投递到 Kafka、RocketMQ 等 MQ，再由同步服务更新 ES。

![Canal 通过 MQ 同步数据](https://oss.javaguide.cn/github/offerkit/database/es/es-mq-synchronization-canal-with-mq.png)
...
![Flink CDC 增量快照衔接 binlog](https://oss.javaguide.cn/github/offerkit/database/es/flink-cdc-incremental-snapshot.webp)
...
![MySQL 同步 ES 的一致性保障](https://oss.javaguide.cn/github/offerkit/database/es/mysql-es-consistency-guardrails.webp)
