...
[AI Agent 基础概念](/ai/agent/agent-basis.html) 中介绍过这条循环。线上故障排查、代码库阅读和测试失败定位都没有预先确定的完整路径，模型需要根据每次拿到的证据调整下一步。
...
![Workflow、Graph、Loop 三者关系概览](https://oss.javaguide.cn/github/offerkit/ai/workflow/workflow-graph-loop-relation.svg)

“生成初稿 → 审核 → 不通过就修改 → 再审核”中，审核不通过的条件边就是从“审核”回到“修改”的回边；审核通过则离开循环。 [AI 工作流中的 Workflow、Graph 与 Loop](/ai/agent/workflow-graph-loop.html) 对这套结构有更完整的说明。运行配置还要写明最大轮次、超时、Token 预算和失败后的降级方式，防止回边没有出口。
...
在 [Harness Engineering](/ai/agent/harness-engineering.html) 中，Agent 可以拆成 Model + Harness。模型负责推理和生成，Harness 提供环境、工具、反馈、沙箱、权限、观测和恢复。
...
`/loop` 按时间再次运行 Prompt，`/goal` 按完成条件决定是否继续。更多说明可以参考 [Claude Code 命令详解](/ai-coding/claudecode-commands.html)。

![Claude Code 推荐使用 loop 命令](https://oss.javaguide.cn/github/offerkit/ai/coding/claudecode/claudecode-father-loop.png)
...
![CI 排查 Loop 示例](https://oss.javaguide.cn/github/offerkit/ai/agent/loop-engineering-ci-triage-loop.webp)
...
![Loop 的安全边界](https://oss.javaguide.cn/github/offerkit/ai/agent/loop-engineering-safety-boundary.webp)
