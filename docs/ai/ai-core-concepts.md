...
description: 汇总 OfferKit AI 专题中的核心概念，按大模型基础、Agent、RAG 和系统设计四条主线串联 Token、Function Calling、Agent Loop、MCP、Skills、多 Agent、Embedding、Rerank、模型网关、评测、可观测性与安全等内容。
category: AI
tag:
  - AI
  - 大模型
...
本文把 OfferKit AI 专题中的相关概念放到一起，具体实现和案例仍由各专题文章展开。
...
![上下文窗口（Context Window）= LLM 的「工作记忆」](https://oss.javaguide.cn/github/offerkit/ai/llm/llm-context-window.png)
...
![Temperature 参数：控制模型输出的随机性](https://oss.javaguide.cn/github/offerkit/ai/llm/llm-temperature-params.png)
...
![Prompt 四要素框架](https://oss.javaguide.cn/github/offerkit/ai/context-engineering/prompt-four-element-framework.svg)
...
![生成阶段三层约束：JSON Mode 管语法，JSON Schema 管契约，Structured Outputs 把契约前移到模型生成阶段](https://oss.javaguide.cn/github/offerkit/ai/llm/structured-output-function-calling-three-layer-constraint.png)
...
![Function Calling 完整调用链路：模型只生成调用意图，真正执行工具的是业务侧](https://oss.javaguide.cn/github/offerkit/ai/llm/structured-output-function-calling-function-calling-pipeline.png)
...
![Agent 记忆分类全景图](https://oss.javaguide.cn/github/offerkit/ai/agent/agent-memory-memory-taxonomy.svg)

记忆系统通常分两层：短期记忆和长期记忆。短期记忆是 Session 级的，服务当前任务；长期记忆跨越 Session，保存需要复用的用户偏好、历史决策和任务经验。两类数据的生命周期、权限和更新方式不同，存储时应在逻辑上分开。

![AI Agent 记忆系统架构](https://oss.javaguide.cn/github/offerkit/ai/agent/agent-memory-arch.png)
...
![长期记忆与 RAG（检索增强生成）的区别](https://oss.javaguide.cn/github/offerkit/ai/agent/agent-memory-rag-vs-memory.svg)
...
![FC/MCP/Agent 三层关系图](https://oss.javaguide.cn/github/offerkit/ai/skills/mcp-fc-agent-layer.png)
...
![ Skill 和 Prompt、MCP、Function Calling 对比](https://oss.javaguide.cn/github/offerkit/ai/skills/skill-prompt-function-calling-mcp-comparison.webp)
...
![Agent = Model + Harness](https://oss.javaguide.cn/github/offerkit/ai/harness/harness-agent-equals-model-harness-arch.png)
...
![Harness 和 Prompt/Context Engineering 的关系](https://oss.javaguide.cn/github/offerkit/ai/harness/harness-engineering-layers-arch.png)
...
![Loop Engineering 外层循环](https://oss.javaguide.cn/github/offerkit/ai/agent/loop-engineering-outer-loop.webp)
...
![RAG 示意图](https://oss.javaguide.cn/github/offerkit/ai/rag/rag-simplified-architecture-diagram.jpeg)
...
![索引和检索阶段的简化流程图](https://oss.javaguide.cn/github/offerkit/ai/rag/rag-rag-engineering-link.png)
...
![Embedding：把文本映射到语义空间](https://oss.javaguide.cn/github/offerkit/ai/rag/rag-2-embedding-map-text-to-semantic-space.png)
...
![Embedding 和向量检索是什么关系？](https://oss.javaguide.cn/github/offerkit/ai/rag/rag-embedding-vector-retrieval.png)
...
![RAG 场景为什么需要向量数据库？](https://oss.javaguide.cn/github/offerkit/ai/rag/rag-why-need-vector-store.png)

### 文档处理

文档从上传到进入向量库，中间要经过至少六个环节：

![RAG 文档处理总链路：上传前半段决定了后半段效果上限](https://oss.javaguide.cn/github/offerkit/ai/rag/rag-document-processing-overall-link.png)
...
![如何选择合适的切分策略？](https://oss.javaguide.cn/github/offerkit/ai/rag/rag-document-processing-chunking-strategy.png)
...
![什么是 GraphRAG？](https://oss.javaguide.cn/github/offerkit/ai/rag/graphrag-simplified-architecture-diagram.png)
...
![GraphRAG 和传统向量 RAG 的本质区别](https://oss.javaguide.cn/github/offerkit/ai/rag/graphrag-vs-rag.png)
